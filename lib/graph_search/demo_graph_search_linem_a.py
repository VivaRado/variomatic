...
def get_agreed_b(_p, _ag_a): #, _ag_b, points_a, points_b
	#
	#ret_matching = []
	#
	for k,v in _ag_a.items():
...
def do_line_check(_a_instance, _b_instance, l_s, l_t, ms_pn_s, mt_pn_s, scp_s, scp_t, _f):
	...
	print(l_t_z)
	print(scp_t[0])
	print(r_c_a) # if any of those points have better match that is in close to the points of the current
...
def do_rad_search(_f, _a_instance, _b_instance, iter_point, _plt=False, _color=False):
	...
	if len(list(points_b.items())) > iter_point:
		...
		if _plt:
			...
			print(l_s)
			#
			if len(l_s) > 2: # if there is a line enough for comparisson
				#
