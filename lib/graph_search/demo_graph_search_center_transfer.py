...
def make_ct(to_ct, cen_c, ax):
	...
	for coords in to_ct:
		...
		ax.add_patch(prp1)
		#
		perps.append([[_perp[0],_perp[1]],[_perp[2],_perp[3]]])
	#
	return perps
	#
...
def do_line_check(_a_instance, _b_instance, l_s, l_t, ms_pn_s, mt_pn_s, scp_s, scp_t, _f, ax, bax, ls_p, lt_p):
	...
	for cts in to_ct:
		...
		for lt in lt_p:
			#
			draw_circle_on_coord([lt[1][0],lt[1][1]], ax, 1, "orange")
			#
			pnt_dist = pnt2line([lt[1][0],lt[1][1]], start, end) # point, start, end
			#
			t_b_dist = math.hypot(cts[0]-lt[1][0], cts[1]-lt[1][1])
			#
			#
			...
			#pnt_dist = pnt_dist+(t_b_dist,)
			#
			#if t_b_dist < 50:
			#
			all_match.append([(lt[2],lt[3]),pnt_dist, [cts[0],cts[1]],[lt[1][0],lt[1][1]]])
			#
				
			#
			prp1 = mpatches.ConnectionPatch([lt[1][0], lt[1][1]],[pnt_dist[2][0],pnt_dist[2][1]],"data", lw=0.2, color="r")
			#
			ax.add_patch(prp1)	
			#
		#
		...
		for x in all_match:
			...
			#
			#t_match = tuple(t_match)
			#
			#draw_circle_on_coord(x[1][2], ax, 1, "orange")
			#
		#
		#sorted_all_match = sorted(all_match, key=lambda x: x[2])[:3]
	...
	#per_ct[1] = sorted(per_ct[1], key=lambda x: x[1][1])
	#
	ctm_Pre = per_ct[0]
	ctm_SC = per_ct[1]
		...
		for z in r_c_a:
			...
			#  	print(mm)
			 ...
			 # 		try:
				...
				# 		pass
					#
			# 		if m[1][0] > mm[1][0]: #and m[1][1] < mm[1][1]:
			# 			#
			# 			#del _ctm[x]
			# 			#if len(ctm_SC) > 1:
			# 				#
			# 			#print("deleted", ctm_SC_c[c])
			# 			#
			# 			try:
			# 				del ctm_SC_c[c]
			# 				ignore_num.add(m[0][0])
			...
			# 			#
			# 			#
			# 			print("FOUND:",m[0],"COMPARED:",m[1][1],'to',mm[1][1])
			# 			#
			# 		#else:
			# 			#
			# 			#print("deleted", ctm_SC[c])
			# 			#
			# 			#del ctm_SC[c]
			# 			#
			# 			#ignore_num.add(m[0][0])
	...
	# 	G1.add_edge(p,x)
	# 	#
	# 	G1.add_node(x,pos=lt_p_rdp[x])
	# 	#
	# 	node_color_map_a.append("blue")
	# 	#
	# #
	# for x in range(len(l_s_rdp)):
	# 	#
	# 	if x == 0:
	# 		#
	# 		p = 0
	...
	# 	G2.add_edge(p,x)
	# 	#
	# 	G2.add_node(x,pos=l_s_rdp[x])
	# 	#
	# 	node_color_map_b.append("red")
	# 	#
	#
	#
	#G1 = json_graph.node_link_graph(self.m_instances[0]["graph_json"])
	#G2 = json_graph.node_link_graph(self.m_instances[1]["graph_json"])
	#
	...
	# nx.draw(G1,pos1, node_color = node_color_map_a)
	# pos2=nx.get_node_attributes(G2,'pos')
	# nx.draw(G2,pos2, node_color = node_color_map_b)
	# #
	# print('==========================')
	# print('==========================')
	# #
	...
	# nx.draw(G3,pos3,node_color = ["green"])
	# #
	# for x in G3.nodes():
	# 	#
	# 	print(x)
...
class MainObject:
	...
	def run(self):
		...
		self.rad_search_box.insert(0,7)
	...
	def draw_topo(self, instance, _plt, i):
		...
		if hide_labels == False:
			#
			nx.draw_networkx_edge_labels(_g,pos,edge_labels=edge_label_map,font_size=7, font_family='monospace', alpha=0.3)
			#
