...
def do_line_check(_a_instance, _b_instance, l_s, l_t, ms_pn_s, mt_pn_s, scp_s, scp_t, _f, ax, bax, ls_p, lt_p, iter_point):
	...
	#
	ev_psca = evaluate_psca(per_ct[0],ctm_SC_c,per_ct[2], iter_point)
	#
...
def plot_region_line(tx, coords, _c, scp, all_points):
	...
	for x in coords:
		...
		if i < len(coords) -1:
			...
			end_num = coords[i][3]
			#
			next_num = get_num(i,coords)[3]
			print("======== NUMBERS", i, next_num)
			#
			if srt_num in scp:
				#
