			...
			for letter in self.instances[instance]:
				...
				for contour in self.instances[instance][letter]:
					...
					try:
						...
						#
						#print("MATCHING")
						#print(t_contour["matching"][_val_smp][_val_pnt])
						#
					...
					try:
						if redraw_pnt:
							...
							#print("REDRAW PNT")
							#
							__point_inx = list(t_contour["graphs"][_val_smp].values())[_val_pnt]["order"]
							__point = list(t_contour["graphs"][_val_smp].values())[_val_pnt]["coord"]
							#
							# graph_point_from_coord = [d for d in list(t_contour["graphs"][_val_smp].values()) if d['coord'] == __point][0]
							# #
							# #
							# confine_from_coord = [d for d in t_contour["confines_simp"][_val_smp] if d[1][1] == __point][0]
							# confine_index = t_contour["confines_simp"][_val_smp].index(confine_from_coord)
							# #
							# #
							# print("current point ---------------------------------")
							# print(__point_inx, graph_point_from_coord, confine_index)
							#
							draw.draw_circle_on_coord(contour_start_point, t_gca, 20, t_color, False)
							#
							...
							for x in t_contour["matching"][_val_smp]:
								#
								print(">>> iter redraw")
								pprint.pprint(x[0][0],__point,x[14])
								#
