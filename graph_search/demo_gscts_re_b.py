...
def barometric(tri):
	...
	#
#
def do_line_check(_a_instance, _b_instance, l_s, l_t, ms_pn_s, mt_pn_s, scp_s, scp_t, _f, ax, bax, ls_p, lt_p, iter_point):
	#
	...
	ctm_Ante = per_ct[2]
	#
	A_bar = barometric([_P[1],_SC[1],_A[1]])
	#
	print("==============")
	print("BAROMETRIC CENTER")
	print(A_bar)
	draw_circle_on_coord(A_bar, bax, 1, "r")
	print("==============")
	#
	triangle_comparisson(ax,bax,l_s, l_t, ctm_Pre,ctm_SC_c,ctm_Ante,to_ct, iter_point, A_bar)
	#
...
def evaluate_psca(_P,_SC,_A, iter_point):
	...
	#
#
def triangle_comparisson(ax,bax,l_s,l_t,ctm_Pre,ctm_SC_c,ctm_Ante, to_ct, iter_point, A_bar):
	...
	for x in ev_psca:
		...
		#
		t1 = plt.Polygon([tl1[1],tl2[1],tl3[1]], color='b',alpha=0.2, linewidth=0.5)
		bax.add_patch(t1)
		#
		print("000000000000000000000000000")
		#
	...
	#move_tri(l1,l2,l3)
	#
	t1 = plt.Polygon([l1,l2,l3], color='r',alpha=0.2, linewidth=0.5)
	bax.add_patch(t1)
	#
	for c in to_ct:
		#
	...
	#
	print("============================================ANGLES")
	Pbcea = angle(A_bar,to_ct[0], True)
	Sbcea = angle(A_bar,to_ct[1], True)
	Abcea = angle(A_bar,to_ct[2], True)
	aPbcea = angle(A_bar,to_ct[0])
	aSbcea = angle(A_bar,to_ct[1])
	aAbcea = angle(A_bar,to_ct[2])
	...
	match_barometric = []
	#
	for t in match_polygons:
		#
		now_bar = barometric(t)
		#
		...
		#
		print("============================================ANGLES M")
		PMbcea = angle(now_bar,t[0], True)
		SMbcea = angle(now_bar,t[1], True)
		AMbcea = angle(now_bar,t[2], True)
		aPMbcea = angle(now_bar,t[0])
		aSMbcea = angle(now_bar,t[1])
		aAMbcea = angle(now_bar,t[2])
		...
		for z in tri_zip:
			#
			is_close.append(math.isclose(z[0],z[1], abs_tol=30))
			#
			print(">>>",math.isclose(z[0],z[1], abs_tol=30) )
			#
		#
		a = np.array(match_ang)
		b = np.array([aPbcea, aSbcea, aAbcea])
		c = a-b
		#$diff_ = difflib.SequenceMatcher(None,[PMbcea,SMbcea,AMbcea],[Pbcea,Sbcea,Abcea])
		#$diff_ratio = diff_.ratio()
		#
		#$print("DIFF", [PMbcea,SMbcea,AMbcea],[Pbcea,Sbcea,Abcea])
		#$print("DIFF RATIO", diff_ratio)
		match_barometric.append([match_polygons.index(t),match_angles, match_diffs, match_tot, match_ang, sum(c), is_close])
		#
		for p in t:
			#
			pp1 = mpatches.ConnectionPatch(now_bar,p,"data", lw=0.5, color="b")
			bax.add_patch(pp1)
			#
		#
	#
	diff_list = []
	#
	sorted_barmatch = sorted(match_barometric, key=lambda x: (x[4] == [aPbcea, aSbcea, aAbcea], x[6]), reverse=True)#(x[1]-1Pbcea, x[1][1]-Sbcea, x[1][2]-Abcea))
	#
	#sorted_barmatch_sm = sorted(sorted_barmatch)#(x[1]-1Pbcea, x[1][1]-Sbcea, x[1][2]-Abcea))
	#
	print("====>>>", [aPbcea, aSbcea, aAbcea])
	pprint.pprint(sorted_barmatch)
	#for x in match_barometric:
		#
	#	ratio = similiarity([Pbcea,Sbcea,Abcea],x)
		#
	#	print(">>>>",ratio)
	
	try:
		
		best_m = match_polygons[match_barometric.index(sorted_barmatch[0])]
		#
		print("best_match=============================")
		...
		draw_circle_on_coord(best_m[1], bax, 10, "b")
...
def do_rad_search(_f, _a_instance, _b_instance, iter_point, _plt=False, _color=False):
	...
	if _plt:
		...
		#
		l_t = get_points_around(item_next_coord,points_a,points_b,l_s, a_items, b_items, _plt, _color, ax, bax)
		#
		...
		#
		do_l_check = do_line_check(_a_instance, _b_instance, l_s, l_t, ms_pn_s, mt_pn_s, scp_s, scp_t, _f, ax, bax, ls_ploted, lt_ploted, iter_point)
		#
		print("RESULTS")
		print(do_l_check)
		#
...
class MainObject:
	...
	def run(self):
		...
		self.rad_search_box.insert(0,21)
