...
def do_line_check(_a_instance, _b_instance, l_s, l_t, ms_pn_s, mt_pn_s, scp_s, scp_t, _f, ax, bax, ls_p, lt_p, iter_point, ghost_points):
	...
	ctm_Ante = per_ct[2]
	#
	A_bar_orig = barometric([_P[1],_SC[1],_A[1]])
	#
	print("==============")
...
def triangle_comparisson(ax,bax,l_s,l_t,ctm_Pre,ctm_SC_c,ctm_Ante, to_ct, iter_point, A_bar_orig, ghost_points):
	...
	for t in match_polygons:
		...
		#
		m_tri = move_tri([l1,l2,l3], diff_bar)
		A_bar = move_tri([A_bar_orig], diff_bar)[0]
		#
		t1 = plt.Polygon(m_tri, color='r',alpha=0.2, linewidth=0.5)
		bax.add_patch(t1)
		#
		#
		...
		for s_p in m_tri:
			...
			pp1 = mpatches.ConnectionPatch(A_bar,s_p,"data", lw=0.5, color="r")
			bax.add_patch(pp1)
			#
		# for c in to_ct:
		# 	#
	...
	try:
		...
		if check_not_ghost == False:
			#
			draw_circle_on_coord(best_m[1], bax, 10, "g")
			#
		else:
			#	
			draw_circle_on_coord(best_m[1], bax, 10, "r")
			#
...
class MainObject:
	...
	def run(self):
		...
		self.rad_search_box.insert(0,8)
...
if __name__ == '__main__':
	#
	#_p = pip(gd_garamond_reg, gd_garamond_bld)
