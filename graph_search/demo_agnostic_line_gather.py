...
def triangle_comparisson(ax,bax,l_s,l_t,ctm_Pre,ctm_SC_c,ctm_Ante, to_ct, iter_point, A_bar_orig, ghost_points):
	...
	for t_tri in match_polygons:
		...
		c = a-b
		#
		match_barometric.append([match_polygons.index(t_tri),match_angles, match_diffs, match_tot, match_ang, sum(c), is_similar_angle, is_sc_distance, abs(s_area - t_area), tt_barextlen, tt_barextlen[1]])
		# #
		for p in t_tri:
			#
	...
	diff_list = []
	#
	sorted_barmatch = sorted(match_barometric, key=lambda x: (x[4] == [aPbcea, aSbcea, aAbcea], x[9] == st_barextlen, x[10] == st_barextlen[1], x[6]), reverse=True)#(x[1]-1Pbcea, x[1][1]-Sbcea, x[1][2]-Abcea))
	#
